(() => {
  ...
  // The drawer toggle -- the button, its state, persistence and the swipe
  // -- is owned outright by ui/sidebar-toggle.js. The delegated capture-
  // phase safety net that used to live here (and the observer, the
  // reassert timers and the second copy of the collapsed state that came
  // with it) existed because two other modules could each leave the
  // button inert; with one owner and one listener there is nothing left
  // to catch, and its stopImmediatePropagation() on the way past was
  // itself a way for a click to go missing.
